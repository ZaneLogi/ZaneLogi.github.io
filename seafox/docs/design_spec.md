...
- `color` is written from the sprite's **colour bitmap**, which can extend **one pixel
  to the right** of the ink and never to the left (§ 6.3 specifies why).
...
not every non-zero colour pixel is ink — § 6.3's chroma cell places colour one pixel to
the right of the silhouette. Treating `color != 0` as the silhouette makes every
isolated-pixel edge one pixel wider for collision purposes.
...
| `color` | `w′ × h` palette indices, where `w′` may be `w + 1` — § 6.3 |
...
1. **Chroma.** For each lit pixel whose left and right neighbours are both unlit, write
   ...
   column is within the row and unlit. This two-pixel cell is why an every-other-pixel
   fill reads as solid colour rather than as stripes.
...
**The chroma cell is the reason `color` can be one pixel wider than `ink`, and only ever
on the right.** An implementation that sizes `color` to the ink's bounding box will clip
the right-hand half of any edge chroma cell.
...
- **No colour decisions.** Each sprite already carries the palette indices it draws in,
  ...
  them.
- **Use the sprite's `color` array, not its `ink`.** They are different shapes — the
  colour bitmap can extend one pixel to the right (§ 6.3).
