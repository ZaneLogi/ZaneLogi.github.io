...
const path_001D = new Uint8Array([
    ...
    0xF7, 0x4B, 0x00,                    // JUMP→ p_flv_004B
    0x23, 0xF0, 0x02,
    0xF0, 0x5E, 0x00,                    // JUMP→ p_flv_005E
...
const path_0067 = new Uint8Array([
    ...
    0xF7, 0x84, 0x00,                    // JUMP→ p_flv_0084
    0x23, 0x16, 0x03,
    0xF0, 0x97, 0x00,                    // JUMP→ p_flv_0097
...
const path_009F = new Uint8Array([
    ...
    0xF7, 0xB6, 0x00,                    // JUMP→ p_flv_00B6
    0x23, 0xF0, 0x08,
    0xF0, 0xCC, 0x00,                    // JUMP→ p_flv_00CC
...
const path_00D4 = new Uint8Array([
    ...
    0xF7, 0x60, 0x01,                    // JUMP→ p_flv_0160
    0x44, 0x16, 0x03,
    0xF0, 0x73, 0x01,                    // JUMP→ p_flv_0173 (stage 13)
...
const path_017B = new Uint8Array([
    ...
    0xF7, 0x92, 0x01,                    // JUMP→ p_flv_0192
    0x44, 0xF0, 0x08,
    0xF0, 0xA8, 0x01,                    // JUMP→ p_flv_01A8
...
const path_01B0 = new Uint8Array([
    ...
    0xF7, 0xCA, 0x01,                    // JUMP→ p_flv_01CA
    0x23, 0x16, 0x03,
    0xF0, 0xE0, 0x01,                    // JUMP→ p_flv_01E0
    0x23, 0x16, 0x1D, 0xFB, 0x23, 0x00, 0xFF, 0xFF,
]);

// ── F0 (ATTACK_WAVE) jump-target sub-paths ─────────────────────────────
// Each token-bearing fly-in path above carries an F0 token (case_0955,
// gg1-5.s:1529) that, FROM STAGE 8 (gate ds_new_stage_parms[0x08] != 0),
// JUMPs to one of these — pointer-replace, no return. Below stage 8 the gate
// is off and F0 skips, byte-identical to the Z80's l_0963 skip; that's why
// stages 1-3 + challenge never touch these. In ROM the sub-paths live in the
// gaps between the main paths, interleaved with non-path data (RST vectors +
// the d_003B task table at 0x34-0x4A, the 96-byte sprt_fmtn_hpos LUT at
// 0x100-0x15F), so a contiguous region would be ~120 bytes of filler. Instead
// each main path carries a `.subPaths` map (Z80 addr → array) that bugMotion's
// F0 handler looks the target up in. Every sub-path is one segment + FB
// (turn-home) + tail — all already-ported tokens. (F7's targets are
// deliberately NOT here: F7 stays deferred — it needs the transient-launch
// layer + the FE token. See research_attack_paths.md §8.)
// Source: gg1-5.s:140,156,170,199,213,227.
const path_005E = new Uint8Array([0x44, 0xE4, 0x18, 0xFB, 0x44, 0x00, 0xFF, 0xFF]); // F0 of db_flv_001d
const path_0097 = new Uint8Array([0x44, 0x27, 0x0E, 0xFB, 0x44, 0x00, 0xFF, 0xFF]); // F0 of db_flv_0067
const path_00CC = new Uint8Array([0x33, 0xE0, 0x10, 0xFB, 0x44, 0x00, 0xFF, 0xFF]); // F0 of db_flv_009f
const path_0173 = new Uint8Array([0x66, 0x20, 0x14, 0xFB, 0x44, 0x00, 0xFF, 0xFF]); // F0 of db_flv_00d4
...
// ── Bonus-bee X3 convoy region — db_0473..p_flv_0502 (gg1-5.s:370-420) ──
// The 3 bonus-bee LEADER paths (one per stage color) plus their CLONE sub-paths,
// ported as ONE contiguous array exactly as the ROM lays them out (0x0473-0x0512),
// z80Base = 0x0473. So the F2 SPAWN tokens and in-region FD jumps resolve within
// it via the usual z80Base offset math (bugMotion). The leaders' home-tails jump
// to 0x0358/0363/039e (the bee descent in ATTACK_PATH_YELLOW, OUTSIDE this region)
// — bugMotion treats an out-of-array FD/FA target as TURN_HOME (the bee descent
// they target ends in FB-home anyway). research_bonus_bee.md §6.
export const CONVOY_REGION = new Uint8Array([
    // db_0473 (color 1) — offset 0
    0x12,0x18,0x1e, 0x12,0x00,0x08, 0xf2,0x99,0x04, 0x00,0x00,0x0a, 0xf2,0x99,0x04,
    0x00,0x00,0x0a, 0x12,0x00,0x2c, 0x12,0xfb,0x26, 0x12,0x00,0x02, 0xfc,0x2e,
    0x12,0xfa,0x3c, 0xfa,0x9e,0x03, 0xfd,0x63,0x03,
    // p_flv_0499 (clones of color 1) — offset 38
    0x12,0x00,0x2c, 0x12,0xfb,0x26, 0x12,0x00,0x02, 0xfc,0x2e, 0x12,0xfa,0x18,
    0x12,0x00,0x10, 0xff,
    // db_04AB (color 2) — offset 56
    0x12,0x18,0x13, 0xf2,0xc6,0x04, 0x00,0x00,0x08, 0xf2,0xcf,0x04, 0x00,0x00,0x08,
    0x12,0x18,0x0b, 0x12,0x00,0x34, 0x12,0xfb,0x26, 0xfd,0x58,0x03,
    // p_flv_04c6 (color-2 clone 1) — offset 83
    0x12,0x00,0x10, 0x12,0x18,0x0b, 0xfd,0xd8,0x04,
    // p_flv_04cf (color-2 clone 2, falls into 04d8) — offset 92
    0x12,0x00,0x08, 0x12,0x18,0x0b, 0x12,0x00,0x06,
    // p_flv_04d8 (shared clone tail) — offset 101
    0x12,0x00,0x22, 0x12,0xfb,0x26, 0x12,0x00,0x02, 0xfc,0x2e, 0x12,0xfa,0x18,
    0x12,0x00,0x20, 0xff,
    // db_04EA (color 0) — offset 119
    0x12,0x18,0x1e, 0x12,0x00,0x14, 0xf2,0x02,0x05, 0x12,0x00,0x08, 0xf2,0x02,0x05,
    0x12,0x00,0x18, 0x12,0xfb,0x26, 0xfd,0x58,0x03,
    // p_flv_0502 (clones of color 0) — offset 143
    0x12,0xe2,0x01, 0xf3, 0x08,0x07,0x06,0x05,0x04,0x03,0x02,0x01, 0xf5,
    0x23,0x00,0x48, 0xff,
]);
CONVOY_REGION.z80Base = 0x0473;

// d_1B5F leader entry offsets by bonus-bee color index (gg1-2_fx.s:841-844):
//   color 0 → db_04EA (offset 119) · color 1 → db_0473 (0) · color 2 → db_04AB (56)
const CONVOY_ENTRY = [119, 0, 56];

// Leader path for a bonus-bee color index (0/1/2): the CONVOY_REGION bytes + the
// leader's entry offset. research_bonus_bee.md §6.
export function getConvoyPath(colorIndex) {
    return { bytes: CONVOY_REGION, entryOffset: CONVOY_ENTRY[colorIndex] ?? 0 };
}
...
// ── d_combat_stg_dat — per-stage fly-in caravans (gg1-3.s:1461) ───────
// 13 unique caravan patterns. Row = 18 bytes:
//   [0,1]   2-byte header (transient-attack control — currently unused)
...
const D_COMBAT_STG_DAT = new Uint8Array([
    ...
    0x14,0x01, 0x82,0x00,0x40+0x80, 0x00,0x01,0x01+0x00, 0x00,0x41,0x41+0x00, 0x02,0x40,0x40+0x00, 0x02,0x00,0x00+0x00, 0xFF,
    0x14,0x01, 0x82,0x02,0x42+0x80, 0x00,0x03,0x05+0x80, 0x00,0x43,0x45+0x80, 0x02,0x42,0x44+0x80, 0x02,0x02,0x04+0x80, 0xFF,
    0x14,0x01, 0x82,0x00,0x40+0x80, 0x00,0x01,0x41+0x80, 0x00,0x41,0x01+0x80, 0x02,0x40,0x00+0x80, 0x02,0x40,0x00+0x80, 0xFF,
    0x14,0x01, 0x82,0x00,0x40+0x80, 0x42,0x01,0x01+0x00, 0xF2,0x41,0x41+0x00, 0x02,0x40,0x40+0x00, 0x02,0x00,0x00+0x00, 0xFF,
    0x14,0x01, 0xA4,0x02,0x42+0x80, 0x52,0x03,0x05+0x80, 0xF2,0x43,0x45+0x80, 0x02,0x42,0x44+0x80, 0x02,0x02,0x04+0x80, 0xFF,
    0x14,0x01, 0x82,0x00,0x40+0x80, 0x52,0x01,0x41+0x80, 0xF2,0x41,0x01+0x80, 0x02,0x40,0x00+0x80, 0x02,0x40,0x00+0x80, 0xFF,
    0x14,0x01, 0xA4,0x00,0x40+0x80, 0x42,0x01,0x01+0x00, 0xF4,0x41,0x41+0x00, 0x04,0x40,0x40+0x00, 0x04,0x00,0x00+0x00, 0xFF,
    0x14,0x01, 0xA4,0x02,0x42+0x80, 0x52,0x03,0x05+0x80, 0xF4,0x43,0x45+0x80, 0x04,0x42,0x44+0x80, 0x04,0x02,0x04+0x80, 0xFF,
    0x14,0x03, 0xA4,0x00,0x40+0x80, 0x54,0x01,0x41+0x80, 0xF4,0x41,0x01+0x80, 0x04,0x40,0x00+0x80, 0x04,0x40,0x00+0x80, 0xFF,
    0x14,0x03, 0xA4,0x00,0x40+0x80, 0x54,0x01,0x01+0x00, 0xF4,0x41,0x41+0x00, 0x04,0x40,0x40+0x00, 0x04,0x00,0x00+0x00, 0xFF,
    0x14,0x03, 0xA4,0x02,0x42+0x80, 0x54,0x03,0x05+0x80, 0xF4,0x43,0x45+0x80, 0x04,0x42,0x44+0x80, 0x04,0x02,0x04+0x80, 0xFF,
]);

// ── d_combat_stg_dat_idx — (rank × stage) → caravan-row offset (gg1-3.s:1437) ─
// 4 ranks × 17 entries; each entry is a byte offset into D_COMBAT_STG_DAT (a
// multiple of 18). IMPORTANT: the fly-in indexes by `rank * 17` DIRECTLY
// (gg1-3.s:1197) — NOT through the bmbr_stg_cfg_lut rotation the difficulty
// table uses. So the clone's rank 3 reads idx ROW 3 here, but difficulty
// sub-table 0. (Stage 1 → entry 0 = 0x00 → caravan row 0, all ranks.)
const D_COMBAT_STG_DAT_IDX = new Uint8Array([
