...
| 11 | Birds (`$3400`) — stages 4-7 | 🚧 | Bird-stage research landed (`research_bird_stage.md`, 2026-05-17) — covers `$3400` dispatch, `$32B0` init, M4368 maturity, T3F80/T3FC0 tables, `$38E9` wing-hit wiring, plus `$3930`/`$25B7` bird-fire path (§8, corrected 2026-05-17). Sub-steps 11.1–11.6 defined in `research_bird_stage.md §9.1` (bird init → combat dispatch → update engine → hit detection → stage-clear cleanup → spiral-fill intro last). **11.7 bird-fire bullets (2026-05-17):** ported `$3930` as `birdFireScanAndSpawn` reusing the existing `enemyBullets[]` pool; wired into both even-parity branches of `stageBirdCombat` (BirdsLeft<4 and BirdsLeft>=4); rate gate at counter9a bit 1, X-band derived from `mappedPlayerX()` ± D. Initial trace at §8 missed `$3930` and claimed birds don't fire — corrected after Zane caught it from arcade video. Port deviation: `$4BD2` (bird-fire subset index) substituted with `counter9a` since M4BD0+ extended-storage state machine isn't ported (§8.6). Debug-start affordance at §9.0 (LevelAndRound override) lets iteration jump directly to stage 5, skipping the alien-combat warm-up. Reuses step-10.7 bonus-explosion machinery for wing/body hits. |
