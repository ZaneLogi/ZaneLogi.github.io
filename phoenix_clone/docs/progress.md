...
| 4 | Input poll + main-loop skeleton (empty handlers) | 🚧 partial | `main.js`/`runloop.js`/`states.js` skeleton landed; per `research_code_flow.md` §5.5 still need to verify state 0/1/2/3 each consume exactly one frame for seeding work |
| 5 | State 0 → 1 (score flash) → 2 (stage 0 init); aliens render at fixed positions | ⏳ | Pulls in `research_stage_structure.md` §4 |
