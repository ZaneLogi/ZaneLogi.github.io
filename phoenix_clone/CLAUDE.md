...
breadth-first scans** of `Code.md` — "find every collision routine",
"list all callers of `$0CC4`", "audit which `L3xxx` paths are ported."
...
The address-citation convention (every claim cites a `Code.md` line
or `Lxxxx` label) is what makes sub-agent output recoverable. Always
demand it in the prompt:
...
see why it's true, **re-derive it from the primary source**. This
session (DrawShields + collision audit, 2026-05-20):
- An audit agent flagged `$39F0` as the bird-vs-player path. Reading
  ...
  pointer was off — re-deriving from `Code.md` found the real
...
falsified when a later port step actually depends on it. This
project has seen at least three of these:
...
- `states.js stageBirdCombat` carried `// TODO 11.x: $3980 (cosmetic)`
  for weeks; `$3980` is the actual bird-kills-player path, not
  cosmetic.

All three only surfaced when DrawShields was being implemented and
the doc claims got tested against real gameplay. **The defense is
the address-citation rule plus this skepticism: if a doc says
something happens "always" or "never" in source, demand the address
it doesn't happen at and re-check.**
...
session of work. The DrawShields work in this project landed as two
commits (mixin-refactor, then feature+audit) for exactly this
reason — if the audit had revealed DrawShields was fundamentally
broken, the refactor would survive the revert.
